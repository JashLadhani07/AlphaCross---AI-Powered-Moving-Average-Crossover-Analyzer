...
- **Dark, responsive React dashboard** with three views: Single Stock, NSE 500 Screener, and Universe Backtest.
...
│   │   ├── nse500_fetcher.py    # Fetches official Nifty 500 list from niftyindices.com (cached, with fallback)
...
### Data Source
- `yfinance`, with `.NS` suffix auto-appended, `history(period=...)`, and automatic retry (3 attempts) on transient failures or missing OHLCV columns.
- NSE 500 constituent list is fetched live from `niftyindices.com` (`ml/nse500_fetcher.py`), cached in-process, with a small hardcoded fallback list if the network call fails.
